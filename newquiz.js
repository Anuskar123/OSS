...
const newqQuizQuestions = [
  ...
  },
  {
    ...
    correctAnswer: 3,
    explanation: `<strong>Priority Scheduling Overview:</strong><br>
    Priority scheduling is a versatile CPU scheduling algorithm that can be implemented in various operating system environments:
    
    <br><br><strong>Key Characteristics:</strong>
    • <strong>Pre-emptive Priority Scheduling:</strong> Higher priority processes can interrupt lower priority ones
    • <strong>Non-pre-emptive (Cooperative) Priority Scheduling:</strong> Running process completes before priority comparison
    • <strong>Dynamic vs Static Priorities:</strong> Can change during execution or remain fixed
    
    <br><br><strong>Implementation Flexibility:</strong>
    Works with both pre-emptive and cooperative systems because the priority mechanism is independent of the scheduling policy. The algorithm simply selects the highest priority ready process.
    
    <br><br><strong>Common Issues:</strong>
    • <strong>Starvation:</strong> Low priority processes may never execute
...
// NEWQ Quiz State Management
let newqCurrentQuestion = 0;
let newqUserAnswers = [];
let newqScore = 0;
let newqQuizStarted = false;

// Initialize NEWQ Quiz
function initializeNewqQuiz() {
    // Set total questions count
    const totalQuestionsElement = document.getElementById('total-questions');
    if (totalQuestionsElement) {
        totalQuestionsElement.textContent = newqQuizQuestions.length;
    }
    ...
    if (startButton) startButton.addEventListener('click', startNewqQuiz);
    if (nextButton) nextButton.addEventListener('click', nextNewqQuestion);
    if (prevButton) prevButton.addEventListener('click', previousNewqQuestion);
    if (submitButton) submitButton.addEventListener('click', submitNewqQuiz);
    if (showAnswerButton) showAnswerButton.addEventListener('click', showNewqAnswer);
    if (continueButton) continueButton.addEventListener('click', continueNewqQuiz);
    if (restartButton) restartButton.addEventListener('click', restartNewqQuiz);
    if (backToIntroButton) backToIntroButton.addEventListener('click', showNewqIntro);
    if (backToMainButton) backToMainButton.addEventListener('click', () => {
        window.location.href = 'index.html';
    });

    console.log('NEWQ Quiz initialized with', newqQuizQuestions.length, 'questions');
}

// Start NEWQ Quiz
function startNewqQuiz() {
    newqCurrentQuestion = 0;
    newqUserAnswers = [];
    newqScore = 0;
    newqQuizStarted = true;
...
function loadNewqQuestion() {
    const question = newqQuizQuestions[newqCurrentQuestion];
    
    // Update question counter
    document.getElementById('current-question').textContent = newqCurrentQuestion + 1;
    
    // Update progress bar
    const progress = ((newqCurrentQuestion) / newqQuizQuestions.length) * 100;
    ...
    question.options.forEach((option, index) => {
        ...
        optionElement.innerHTML = `
            ...
            <label for="option-${index}">${option.replace(/\n/g, '<br>')}</label>
        `;
        ...
        if (newqUserAnswers[newqCurrentQuestion] !== undefined && newqUserAnswers[newqCurrentQuestion] === index) {
    ...
    // Hide explanation section when loading new question
    ...
    document.getElementById('prev-question').style.display = newqCurrentQuestion === 0 ? 'none' : 'inline-block';
    document.getElementById('next-question').style.display = newqCurrentQuestion === newqQuizQuestions.length - 1 ? 'none' : 'inline-block';
    document.getElementById('submit-quiz').style.display = newqCurrentQuestion === newqQuizQuestions.length - 1 ? 'inline-block' : 'none';
}

// Next question
function nextNewqQuestion() {
    saveCurrentAnswer();
    
    // Hide explanation section when moving to next question
    document.getElementById('explanation-section').style.display = 'none';
    ...
    if (newqCurrentQuestion < newqQuizQuestions.length - 1) {
        newqCurrentQuestion++;
        loadNewqQuestion();
    }
}
...
function showNewqAnswer() {
    ...
    // Save the answer
    newqUserAnswers[newqCurrentQuestion] = userAnswer;
    
    // Show explanation section inline
    ...
    if (isCorrect) {
        resultIndicator.innerHTML = '<div class="result-correct">✅ Correct!</div>';
        resultIndicator.className = 'result-indicator correct';
    } else {
        resultIndicator.innerHTML = `<div class="result-incorrect">❌ Incorrect!</div><div class="correct-answer">The correct answer is: ${question.options[question.correctAnswer]}</div>`;
    ...
    options.forEach((option, index) => {
        ...
        } else if (radio.checked && index !== question.correctAnswer) {
            option.classList.add('incorrect');
        }
        // Disable all options after showing answer
        radio.disabled = true;
        option.style.pointerEvents = 'none';
    });
    
    // Hide show answer button after it's clicked
    document.getElementById('show-answer-btn').style.display = 'none';
}

// Previous question
function previousNewqQuestion() {
    saveCurrentAnswer();
    
    // Hide explanation section when moving to previous question
    document.getElementById('explanation-section').style.display = 'none';
...
function saveCurrentAnswer() {
    const selectedOption = document.querySelector('input[name="answer"]:checked');
    if (selectedOption) {
        newqUserAnswers[newqCurrentQuestion] = parseInt(selectedOption.value);
    }
}

// Submit quiz
function submitNewqQuiz() {
    saveCurrentAnswer();
    
    // Calculate score
    newqScore = 0;
    newqQuizQuestions.forEach((question, index) => {
        if (newqUserAnswers[index] === question.correctAnswer) {
            newqScore++;
        }
    });
...
// Show results
function showNewqResults() {
    document.getElementById('quiz-container').style.display = 'none';
    document.getElementById('results-container').style.display = 'block';
    
    const percentage = Math.round((newqScore / newqQuizQuestions.length) * 100);
    
    document.getElementById('final-score').textContent = newqScore;
    document.getElementById('score-percentage').textContent = percentage + '%';
    document.getElementById('correct-count').textContent = newqScore;
    document.getElementById('incorrect-count').textContent = newqQuizQuestions.length - newqScore;
